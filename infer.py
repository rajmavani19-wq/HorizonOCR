...
def get_ngram_processor_str():
    ...
    if NO_REPEAT_NGRAM_PROCESSOR_STR is None:
        try:
            from sglang.srt.sampling.custom_logit_processor import (  # type: ignore[import-not-found]
                DeepseekOCRNoRepeatNGramLogitProcessor,
            )
            NO_REPEAT_NGRAM_PROCESSOR_STR = DeepseekOCRNoRepeatNGramLogitProcessor.to_str()
        except ImportError:
            print("[Warning] sglang not available — requires Linux + NVIDIA CUDA GPU.")
            NO_REPEAT_NGRAM_PROCESSOR_STR = ""
...
def parse_args():
    ...
    parser.add_argument("--model_dir", default="baidu/Unlimited-OCR",
                        help="HF Hub model ID or path to locally downloaded model")
